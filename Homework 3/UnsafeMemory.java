class UnsafeMemory {
    public static void main(String args[]) {
		...
		try {
		    ...
		    //begin initializing the state object
		    State s;
			...
			//end initializing the state object
		    dowork(nThreads, nTransitions, s); //call dowork()
		    test(value, s.current(), maxval); //call test()
		    System.exit (0);
		} catch (Exception e) {
		    usage(e);
		}
    ...
    private static void dowork(int nThreads, int nTransitions, State s)
      throws InterruptedException {
		Thread[] t = new Thread[nThreads]; //create threads
    ...
    private static void test(byte[] input, byte[] output, byte maxval) {
		...
		for (int i = 0; i < input.length; i++)
		{
			isum += input[i];
			osum += output[i];
			//check every element in output list does not
			//exceed maxval or go below 0
			if (output[i] < 0)
			    error("negative output", output[i], 0);
			if (output[i] > maxval)
			    error("output too large", output[i], maxval);
		}
		//check sum of elements in output is the same as
		//sum of elements in input
		if (isum != osum)
